...
from pinocchio.utils import eye, rotate, se3ToXYZQUAT, zero
...
def loadRobot(M0, name):
    ...
    robot.initDisplay(loadModel=True, sceneName="world/" + name)
    return robot
...
gepettoViewer.addBox('world/robot0/toolplate', w, h, d, color)
Mtool = SE3(rotate('z', 1.268), np.matrix([0, 0, .77]).T)
gepettoViewer.applyConfiguration('world/toolplate', fromSE3(Mtool))
